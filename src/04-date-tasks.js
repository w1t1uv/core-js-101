...
function timeSpanToString(startDate, endDate) {
  ...
  return `${hours < 10 ? `0${hours}` : hours}:${minutes < 10 ? `0${minutes}` : minutes}:${seconds < 10 ? `0${seconds}` : seconds}.${milliseconds < 10 ? `00${milliseconds}` : milliseconds}`;
}
...
function angleBetweenClockHands(date) {
  ...
  let angle = Math.abs(0.5 * ((hours % 12) * 60 - 11 * minutes));
